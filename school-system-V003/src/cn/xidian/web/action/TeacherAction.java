...
public class TeacherAction extends ActionSupport implements RequestAware {
	...
	public String selectStuPer() {
		...
		Set<StudentCourse> stuCurs = studentCourseService
				.selectStuCursLimits(stuCursLimit);
	...
	public String calculateClazzCursTarget() {
		try {
			...
			teacherService.caculateClazzTarget(cursName,
					clazzName, tchrSchNum);
			CourseTargetDetailService courseTargetDetailService = new CourseTargetDetailService();
			List<TeachingTarget> targets = teachingTargetService
					.selectByCursName(cursName);
			List<TeachingTargetEvaluate> targetValues = teachingTargetEvaluateService
					.selectByCursAndClazz(cursName,clazzName);
			claCursB1s = courseTargetDetailService.getB1(targets, targetValues);

			List<ClazzCoursePoint> ccPoints = clazzCoursePointService
					.selectByCursAndClazz(cursName,clazzName);
	...
	public String findB1B2() {
		try {
			...
			List<TeachingTargetEvaluate> ttValue = courseService.selectByCursNameAndGrade(
					cursName,grade);
			...
			for (int i = 0; i < clazzs.size(); i++) {
				...
				for (int j = 0; j < targets.size(); j++) {
					for (int k = 0; k < ttValue.size(); k++) {
						if (ttValue.get(k).getClazz().getClaName()
								.equals(clazzs.get(i))
								&& ttValue.get(k).getTeachingTarget()
										.getTchTargetId() == targets.get(j)
										.getTchTargetId()) {
			...
			// 以上获得第一张表的数据
			// 以下是第二张表
			List<ClazzCoursePoint> cursPoints = clazzCoursePointService
					.selectBycursNameAndTerm(cursName);
			List<CoursePoint> point = coursePointService.selectByCursId(course
					.getCursId());
			// 获得该门课程对应指标点编号的集合
			...
			for (int i = 0; i < clazzs.size(); i++) {
				...
				for (int j = 0; j < points.size(); j++) {
					for (int k = 0; k < cursPoints.size(); k++) {
						if (cursPoints.get(k).getClazz().getClaName()
								.equals(clazzs.get(i))
								&& cursPoints.get(k).getIndPoint()
										.getIndPointNum().equals(points.get(j))) {
	...
	public String selectClazzCursTargetValueByGrade(){
		...
		for (int i = 0; i < isEvaluate.size(); i++) {
			...
			c.setTeacher(ie.getTeacher().getTchrSchNum()
					+ ie.getTeacher().getTchrName());
